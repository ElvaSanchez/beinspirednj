...
export function PlaceholderImage({ id, aspect, label, alt, className = "", shape, loading = "lazy", sizes = "(max-width: 767px) calc(100vw - 40px), (max-width: 1279px) 45vw, 550px" }: { id?: string; aspect?: string; label?: string; alt?: string; className?: string; shape?: "rect" | "circle"; loading?: "lazy" | "eager"; sizes?: string }) {
  ...
  if (image?.available) return <div data-image-path={image.path} className={`placeholder photo-slot ${(shape ?? image.shape) === "circle" ? "placeholder-circle" : ""} ${className}`} style={{ aspectRatio: aspect ?? image.aspect }}>
    <Image src={image.path} alt={alt ?? caption} fill loading={loading} sizes={sizes} style={{ objectFit: "cover" }}/>
  </div>;
